...
RasterizeGaussiansCUDA(
	...
	const torch::Tensor& transparency,
	...
	const bool debug)
{
  ...
  torch::Tensor out_nearest_depth = torch::full({1, H, W}, 0, float_opts);
  torch::Tensor out_transparency = torch::full({1, H, W}, 0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		transparency.contiguous().data_ptr<float>(),
		...
		out_nearest_depth.contiguous().data_ptr<float>(),
...
RasterizeGaussiansBackwardCUDA(
	...
	const torch::Tensor& dL_dout_nearest_depth,
	const torch::Tensor& dL_dout_transparency,
	const torch::Tensor& sh,
	const int degree,
	const torch::Tensor& campos,
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dtransparency = torch::zeros({P, 1}, means3D.options());
  ...
  {  
	  ...
	  dL_dout_nearest_depth.contiguous().data_ptr<float>(),
	  dL_dout_transparency.contiguous().data_ptr<float>(),
	  dL_dmeans2D.contiguous().data_ptr<float>(),
	  dL_dmeans2D_abs.contiguous().data_ptr<float>(),
	  dL_dconic.contiguous().data_ptr<float>(),  
	  dL_dopacity.contiguous().data_ptr<float>(),
	  dL_dtransparency.contiguous().data_ptr<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dmeans2D_abs, dL_dcolors, dL_dopacity, dL_dtransparency, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dall_map);
}
	...
	const bool debug)
{
  ...
  torch::Tensor radii = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  torch::Tensor out_depth_with_transparency = torch::full({1, H, W}, 0, float_opts);
  torch::Device device(torch::kCUDA);
  torch::TensorOptions options(torch::kByte);
  torch::Tensor geomBuffer = torch::empty({0}, options.device(device));
  torch::Tensor binningBuffer = torch::empty({0}, options.device(device));
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forwardDepth(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
		...
		tan_fovy,
		prefiltered,
		out_depth_with_transparency.contiguous().data_ptr<float>(),
		radii.contiguous().data_ptr<int>(),
		start_threshold,
		end_threshold,
		window_size,
