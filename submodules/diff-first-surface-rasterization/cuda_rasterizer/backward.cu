...
renderCUDA(
	...
	const float* __restrict__ dL_dout_nearest_depth,
	const float* __restrict__ dL_dout_transparency,
	float3* __restrict__ dL_dmean2D,
	float3* __restrict__ dL_dmean2D_abs,
	float4* __restrict__ dL_dconic2D,
	float* __restrict__ dL_dopacity,
	float* __restrict__ dL_dtransparency,
	...
	const bool render_geo)
{
	...
	const int transparency_contributor = inside ? n_contrib[pix_id + H * W] : 0;
	float dL_dtransparency_pix;
	if (inside) {
		for (int i = 0; i < C; i++) {
		...
		if(render_geo) {
			...
			// TODO: add gradient w.r.t. nearest depth
			dL_dtransparency_pix = dL_dout_transparency[pix_id];
		}
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (render_geo) {
				...
				float dL_dtrans = 0.0f;
				if (contributor == transparency_contributor - 1){
					dL_dtrans += dL_dtransparency_pix;
				}
				atomicAdd(&(dL_dtransparency[global_id]), dL_dtrans);
			}
...
void BACKWARD::render(
	...
	const float* dL_dout_nearest_depth,
	const float* dL_dout_transparency,
	float3* dL_dmean2D,
	float3* dL_dmean2D_abs,
	float4* dL_dconic2D,
	float* dL_dopacity,
	float* dL_dtransparency,
	float* dL_dcolors,
	float* dL_dall_map,
	const bool render_geo)
{
	renderCUDA<NUM_CHANNELS,NUM_ALL_MAP> << <grid, block >> >(
		...
		dL_dout_all_map,
		dL_dout_plane_depth,
		dL_dout_nearest_depth,
		dL_dout_transparency,
		dL_dmean2D,
		dL_dmean2D_abs,
		dL_dconic2D,
		dL_dopacity,
