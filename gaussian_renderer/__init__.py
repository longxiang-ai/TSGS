...
def render_normal(viewpoint_cam, depth, offset=None, normal=None, scale=1):
    """
    Render the normal from the depth image.
    Args:
        viewpoint_cam: The viewpoint camera.
        depth: The depth image. (H, W)
        offset: The offset of the depth image. (H, W)
        normal: The normal image. (3, H, W)
    Returns:
        normal_ref: The normal image. (3, H, W), data range: [-1, 1]
    """
...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, 
    ...
    hybrid: whether to use the hybrid rendering method, if True, final color = sh_color + mlp_color, else final color = mlp_color
    """
    ...
    transparencies = pc.get_transparency
    ...
    rendered_image, radii, out_observe, out_all_map, plane_depth, out_nearest_depth, out_transparency_map = rasterizer(
        means3D = means3D,
        means2D = means2D,
        means2D_abs = means2D_abs,
        shs = shs,
    ...
    return_dict =  {"render": rendered_image,
                    ...
                    "out_nearest_depth": out_nearest_depth,
                    "out_transparency_map": out_transparency_map
                    }
...
def render_depth(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, 
    ...
    global_normal = pc.get_normal(viewpoint_camera) # world coordinate
    local_normal = global_normal @ viewpoint_camera.world_view_transform[:3,:3] # local camera coordinate
    pts_in_cam = means3D @ viewpoint_camera.world_view_transform[:3,:3] + viewpoint_camera.world_view_transform[3,:3]
    depth_z = pts_in_cam[:, 2]
    local_distance = (local_normal * pts_in_cam).sum(-1).abs()
    input_all_map = torch.zeros((means3D.shape[0], 5)).cuda().float()
