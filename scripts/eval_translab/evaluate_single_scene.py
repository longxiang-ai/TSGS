...
if __name__ == "__main__":
    ...
    parser.add_argument('--TransLab', type=str,  default='data/translab', help='path to the base data directory')
    args = parser.parse_args()

    TransLab_Dataset = args.TransLab
    ...
    cull_scan(scan, ply_file, result_mesh_file, instance_dir=os.path.join(TransLab_Dataset, f'scene{args.scan_id}', 'masks'))

    script_dir = os.path.dirname(os.path.abspath(__file__))
    cmd = f"python {script_dir}/eval.py --data {result_mesh_file} --scan {scan} --mode mesh --dataset_dir {TransLab_Dataset} --vis_out_dir {out_dir}"
    os.system(cmd)